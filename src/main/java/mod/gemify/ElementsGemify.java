...
public class ElementsGemify implements IFuelHandler, IWorldGenerator {
	...
	public void preInit(FMLPreInitializationEvent event) {
		try {
			for (ASMDataTable.ASMData asmData : event.getAsmData().getAll(ModElement.Tag.class.getName())) {
				...
				if (clazz.getSuperclass() == ElementsGemify.ModElement.class)
					elements.add((ElementsGemify.ModElement) clazz.getConstructor(this.getClass()).newInstance(this));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		Collections.sort(elements);
		elements.forEach(ElementsGemify.ModElement::initElements);
		this.addNetworkMessage(GemifyVariables.WorldSavedDataSyncMessageHandler.class, GemifyVariables.WorldSavedDataSyncMessage.class, Side.SERVER,
				Side.CLIENT);
	}
	...
	public void onPlayerLoggedIn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.player.world.isRemote) {
			WorldSavedData mapdata = GemifyVariables.MapVariables.get(event.player.world);
			WorldSavedData worlddata = GemifyVariables.WorldVariables.get(event.player.world);
			if (mapdata != null)
				Gemify.PACKET_HANDLER.sendTo(new GemifyVariables.WorldSavedDataSyncMessage(0, mapdata), (EntityPlayerMP) event.player);
			if (worlddata != null)
				Gemify.PACKET_HANDLER.sendTo(new GemifyVariables.WorldSavedDataSyncMessage(1, worlddata), (EntityPlayerMP) event.player);
		}
	}
	...
	public <T extends IMessage, V extends IMessage> void addNetworkMessage(Class<? extends IMessageHandler<T, V>> handler, Class<T> messageClass,
			Side... sides) {
		for (Side side : sides)
			Gemify.PACKET_HANDLER.registerMessage(handler, messageClass, messageID, side);
		messageID++;
	}
	...
	public static class ModElement implements Comparable<ModElement> {
		...
		protected final ElementsGemify elements;
		protected final int sortid;
		public ModElement(ElementsGemify elements, int sortid) {
